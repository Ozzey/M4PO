...
def parse_cfg(cfg):
	...
	for task in cfg.tasks:
		task_metadata = task_info.get(task)
		if task_metadata is None:
			from envs.isaaclab import is_isaaclab_task, probe_task_info

			if not is_isaaclab_task(task):
				raise AssertionError(f'Task {task} not found in task embeddings.')
			task_metadata = probe_task_info(task)

		text_embedding = task_metadata.get('text_embedding', [])
		if cfg.task_dim > 0 and len(text_embedding) == 0:
			raise ValueError(
				f'Task {task} does not provide a language embedding. '
				'Isaac Lab tasks are currently supported only for single-task M3PO runs.'
			)

		cfg.task_embeddings.append(text_embedding)
		cfg.episode_lengths.append(task_metadata['max_episode_steps'])
		if 'discount_factor' in task_metadata:
			cfg.discounts.append(task_metadata['discount_factor'])
		else:
			cfg.discounts.append(discount_heuristic(cfg, task_metadata['max_episode_steps']))
		cfg.action_dims.append(task_metadata['action_dim'])
