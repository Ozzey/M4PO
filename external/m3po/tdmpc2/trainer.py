...
class Trainer():
	...
	def __init__(self, cfg, env, agent, buffer, logger):
		...
		if cfg.rank == 0:
			...
			print(f'Nominal episode horizon: {self.cfg.episode_length:,}')
	...
	def train(self):
		...
		while self._step <= self.cfg.steps:
			...
			self._tds[ep_len] = td
			if done.any():
				self._ep_idx += int(done.sum().item())

				for i in range(self.cfg.num_envs):
					if done[i]:
						assert ep_len[i] <= self._episode_lengths[i], \
							f'Episode length {ep_len[i]} exceeds configured maximum {self._episode_lengths[i]}.'
				...
				if self._step % self._update_freq == 0 and len(train_metrics['episode_reward']) > 0:
