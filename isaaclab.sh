...
setup_conda_env() {
    ...
    if [ -f "${isaacsim_setup_conda_env_script}" ]; then
        ...
        cat >> ${CONDA_PREFIX}/etc/conda/activate.d/setenv.sh <<'EOF'
# Source builds may keep USD's Python bindings in the Packman cache instead of the runtime tree.
# If pxr is still missing after sourcing Isaac Sim's env script, fall back to the first cached USD path.
if ! python - <<'PY' >/dev/null 2>&1
import pxr
PY
then
    _isaacsim_usd_init=$(find "${HOME}/.cache/packman/chk" -path '*/lib/python/pxr/__init__.py' -print -quit 2>/dev/null)
    if [ -n "${_isaacsim_usd_init}" ]; then
        _isaacsim_usd_python=$(dirname "$(dirname "${_isaacsim_usd_init}")")
        case ":${PYTHONPATH:-}:" in
            *":${_isaacsim_usd_python}:"*) ;;
            *) export PYTHONPATH="${_isaacsim_usd_python}${PYTHONPATH:+:${PYTHONPATH}}";;
        esac
        unset _isaacsim_usd_python
    fi
    unset _isaacsim_usd_init
fi
EOF
    fi
